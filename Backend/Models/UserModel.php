...
class UserModel extends Database
{
    ...
    protected function reigisterModel($name, $mobile, $password)
    {
        if (!empty($name) && !empty($mobile) && !empty($password)) {
            ...
            if ($result->num_rows > 0) {
                ...
                $query_insert = "INSERT INTO users (user_name, user_password, user_mobile, user_registered_on, user_verification, user_wallet, user_verify_otp) VALUES ('" . $name . "', '" . $password . "', '" . $mobile . "', '" . $registered_on . "', $verification, $user_wallet, '" . $verify_otp . "')";
    ...
    protected function fetchUserIdFromNumber($userNumber)
    {
        $query = "SELECT user_id FROM users WHERE user_mobile = " . $userNumber . " LIMIT 1;";
        $result = $this->connect()->query($query);
        if ($result->num_rows > 0) {
            $mobile = $result->fetch_row();
            return $mobile[0];
        }
    }
}
